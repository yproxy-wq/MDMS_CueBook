...
### [2026-08-18] v0.98-dev - 同期完了保証・シナリオ競合防止・診断性・App責務分離
- `WriteBloatGuardian` のスロットル中更新をすべて耐久書き込み完了へ連動させ、4回目以降の更新がFirestore書き込み前に成功扱いとなる競合を修正。連続更新と最終書き込み失敗の回帰試験を追加。
- `useSyncEngine` のシナリオ読込へ非同期要求ガードを導入。URLまたは履歴操作で `scenarioId` が連続変更されても、古いIndexedDB読み込みが現在のシナリオや `isReady` を上書きしないようにした。
- `ErrorLogger` のlocalStorage読込に構造検証・正規化・自己修復を追加し、壊れた診断データが新しい障害記録を妨げないようにした。主要なシナリオ／保存／認証／リセット操作は、マスク済みの構造化診断コードで記録する。
...
- Bizの品質ゲートにはFunctionsを配備せず、ルート型検査に必要な依存関係だけを事前取得する手順を追加。
...
### [2026-08-04] portal.1 - CueBookポータルサイト試作を分離追加
- **新規ポータル (`portal/`)**: 既存アプリのコード・ビルド環境とは分離した静的ポータルを追加。縦レール、同期ビジュアル、Stable版CTA、マニュアル導線を2画面程度のスクロールに収めた。
- **デザイン**: HallmarkのSplit Studio／side-rail構成と、CueBookのdark・technical・brutalist・elegant方針を採用。提供された暫定ロゴ、ペン先、同期画面素材を `portal/assets/` に配置。
- **レスポンシブ**: 320px以上で横スクロールを発生させず、モバイルでは縦レールをコンパクトな上部バーへ変形する。
...
### [2026-08-04] v0.98-dev.4 - アクティブシナリオの単一化を明示
- **進行管理 (`App.tsx`, `ScenarioManagerModal.tsx`)**: 進行データはシナリオ別に並列保持しつつ、1画面でアクティブになるシナリオは常に1つであることをUIとマニュアルに明記。切り替え前保存と`CURRENT`表示を基準にする。
...
# [2026-08-05] v0.98-dev - マイシナリオ切り替えデモ用シナリオ追加
